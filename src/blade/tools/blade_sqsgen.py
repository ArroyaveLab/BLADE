...
class BladeSQS:
    ...
    def sqs_gen(
        ...
        Args:
            ...
            params (dict): ``mcsqs`` run parameters. Required keys:
                ...
                - ``"stop_grace"`` (int, optional): grace period in seconds
                  after ``stopsqs`` before force-kill. Defaults to ``30``.
        """
    ...
    def _wait_with_timeout(
        ...
        Args:
            ...
            params (dict): Must contain ``"time"`` and optionally
                ``"stop_grace"`` (default ``30`` s).
        """
        start_time = time.time()
        ...
        grace = params.get("stop_grace", 30)
        ...
        for ip, p in processes:
            if p.poll() is None:
                print(f"mcsqs -ip={ip} ignored stopsqs; terminating")
                p.terminate()
        ...
        for ip, p in processes:
            if p.poll() is None:
                print(f"mcsqs -ip={ip} still running; killing")
                p.kill()
